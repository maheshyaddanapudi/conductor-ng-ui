...
import { MetadataManagementService, TaskDef, WorkflowDef, WorkflowTask } from 'src/app/Rest/Conductor';
import { NavigatorVarHolderService } from 'src/app/Services/Holders/navigator-var-holder.service';

@Component({
  selector: 'app-initiate-simple-conductor-workflow-creation',
  templateUrl: './initiate-simple-conductor-workflow-creation.component.html',
  styleUrls: []
})
export class InitiateSimpleConductorWorkflowCreationComponent implements OnInit {
  ...
  async proceed_to_next_stage(){
    ...
    this.router.navigateByUrl('define-simple-conductor-workflow-creation', { skipLocationChange: true },).then((fulfilled: boolean) => {
      console.log('Routed')
      });
  }
