...
@Component({
  selector: 'app-update-taskdef',
  templateUrl: './update-taskdef.component.html',
  styleUrls: []
})
export class UpdateTaskdefComponent implements OnInit {
  ...
  constructor(private router: Router, private modalService: NgbModal, private metadataManagementService: MetadataManagementService, private navigatorVarHolderService: NavigatorVarHolderService) { 
    this.inputKeys = [];
    this.outputKeys = [];
  }
  ...
  async ngOnInit() {
    ...
    let task_name = this.navigatorVarHolderService.get_task_name();
