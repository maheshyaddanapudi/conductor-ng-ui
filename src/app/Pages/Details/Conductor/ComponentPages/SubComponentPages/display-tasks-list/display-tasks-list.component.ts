...
import { MetadataManagementService, Task, TaskDef, WorkflowTask } from 'src/app/Rest/Conductor';
...
export class DisplayTasksListComponent implements OnInit {
  ...
  constructor(private modalService: NgbModal, private router: Router, private metadataManagementService: MetadataManagementService) { }

  async ngOnInit() {
    await this.update_task_defs()
  }

  get_keys_length(inputParameters: any):number{
    return Object.keys(inputParameters).length
  }
  ...
  get_input_data_from_execution(a_task: WorkflowTask){
    ...
    this.tasks_execution_list.forEach((exec_task: Task)=>{
      ...
      {
        input_data = exec_task.inputData;
      }
    })
  ...
  get_status_from_execution(a_task: WorkflowTask){
   ...
   get_output_data_from_execution(a_task: WorkflowTask){

    let output_data: any 

    let output_data_extracted: boolean = false

     this.tasks_execution_list.forEach((exec_task: Task)=>{
      if(exec_task.referenceTaskName == a_task.taskReferenceName)
      {
        output_data = exec_task.outputData;
        output_data_extracted = true;
      }
    })

    if(output_data_extracted)
      {
        return output_data
      }
    else
  ...
  async open_tasks_in_new_window(parent_task_ref_name: string){
    ...
    let url = this.router.createUrlTree(['/display-branches-list'], { skipLocationChange: true})
