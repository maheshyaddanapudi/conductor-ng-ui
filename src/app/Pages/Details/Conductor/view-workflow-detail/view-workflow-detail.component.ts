...
@Component({
  selector: 'app-view-workflow-detail',
  templateUrl: './view-workflow-detail.component.html',
  styleUrls: []
})
export class ViewWorkflowDetailComponent implements OnInit {
  ...
  async open_diagram_in_new_window(){
    let url = this.router.createUrlTree(['/view-workflow-diagram'], { skipLocationChange: true})
    window.open(url.toString(), '_blank')
  }

  async open_json_in_new_window(){

    localStorage.setItem('fillory_ui_buffer_branch_workflow_def_json', this.workflow_def_json)
  ...
  async show_current_executions(){
    ...
    await this.router.navigateByUrl('view-individual-workflow-running-executions', { skipLocationChange: true },).then((fulfilled: boolean) => {
      console.log('Routed')
      });
  }
  ...
  async show_all_executions(){
    ...
    await this.router.navigateByUrl('view-individual-workflow-executions', { skipLocationChange: true },).then((fulfilled: boolean) => {
      console.log('Routed')
      });
  }
