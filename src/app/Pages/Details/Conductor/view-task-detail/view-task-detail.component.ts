...
export class ViewTaskDetailComponent implements OnInit {
  ...
  constructor(private taskManagementService: TaskManagementService, private router: Router, private modalService: NgbModal, private navigatorVarHolderService: NavigatorVarHolderService, private metadataManagementService: MetadataManagementService) { }

  async ngOnInit() {
    this.task_name = this.navigatorVarHolderService.get_task_name()
  ...
  async delete(){
    this.metadataManagementService.unregisterTaskDef(this.task_name).toPromise().then((response: any) => {
      ...
      const time_out_timer = setTimeout(async ()=> {
        ...
        await this.router.navigateByUrl('view-taskdefs', { skipLocationChange: true }).then(async (fulfilled: boolean) => {
  ...
  async update(){
    await this.router.navigateByUrl('update-taskdef', { skipLocationChange: true },).then((fulfilled: boolean) => {
      console.log('Routed')
      });
  }
