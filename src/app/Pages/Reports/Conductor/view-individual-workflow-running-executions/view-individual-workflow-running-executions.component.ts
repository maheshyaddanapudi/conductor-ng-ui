...
import { WorkflowManagementService, Workflow, WorkflowBulkManagementService, BulkResponse } from 'src/app/Rest/Conductor';
...
@Component({
  selector: 'app-view-individual-workflow-running-executions',
  templateUrl: './view-individual-workflow-running-executions.component.html',
  styleUrls: []
})
export class ViewIndividualWorkflowRunningExecutionsComponent implements OnInit {
  ...
  constructor(private router: Router, private workflowManagementService: WorkflowManagementService, private workflowBulkManagementService: WorkflowBulkManagementService, private modalService: NgbModal, private navigatorVarHolderService: NavigatorVarHolderService) { 
  ...
  async ngOnInit() {
    ...
    this.workflow_version = this.navigatorVarHolderService.get_workflow_version()
  ...
  {
    this.navigatorVarHolderService.set_workflow_details(this.workflow_name, this.workflow_version)
    
    await this.router.navigateByUrl('view-workflow-detail', { skipLocationChange: true },).then((fulfilled: boolean) => {
      console.log('Routed')
      });
  }
